...
function naverDateToISO(pubDate) {
  // 네이버 API의 pubDate 형식: "Mon, 26 Jan 2026 09000:00 +0900" 같은 RFC1123 포맷
...
async function fetchBrandNews(brand) {
  const url = `https://openapi.naver.com/v1/search/news.json?query=${encodeURIComponent(brand)}&display=${DISPLAY_PER_BRAND}&sort=date`;
...
async function main() {
  ...
  for (const brand of BRANDS) {
    ...
    // 네이버 API 초당 호출 제한을 여유있게 피하기 위한 짧은 대기
